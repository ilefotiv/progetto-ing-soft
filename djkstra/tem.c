...
edge* complete_array_edge(int num_nodes) {
	edge* edge_out;
	int i, j, k;
	edge_out = (edge*) malloc(
			(num_nodes * (num_nodes - 1) / 2 - 1) * sizeof(edge));
	k = 0;
	for (i = 0; i < num_nodes; i++) {
		for (j = 0; j < i; j++, k++) {
			edge_out[k].node1 = i;
...
adj_list* new_erdos_graph(double p, int num_nodes, int max_value) {
	...
	int i, j, r, p_big = (int) (p * RAND_MAX);
	max_value++;
...
adj_list* new_unif_graph(int num_nodes, int num_edges, edge* edge_in,
		int max_value) {
	int i, num_nodes_curr, w, pick;
	adj_list* adj_list_out;
...
heap* new_heap(adj_list* adj_list_in, int source, int num_sons) {
	heap* heap_out;
	node_dist* node_dist_in;
	int i, num_nodes = adj_list_in->num_nodes;
...
void print_heap(heap* heap_in) {
	int i;
	for (i = 0; i < heap_in->num_nodes; i++)
		printf("(%d,%d,%d)", heap_in->ek[i].node, heap_in->ek[i].dist,
				heap_in->ek[i].sort);
	printf("\n");
}
...
node_dist extract_min(heap* heap_in) {
	...
	int sort1 = 0, sort2, i, first_son1, last_son1,
			num_sons = heap_in->num_sons;
	...
	while (first_son1 < num_nodes) {
		sort2 = first_son1;
		if (first_son1 + num_sons < num_nodes)
			last_son1 = first_son1 + num_sons;
		else
			last_son1 = num_nodes;
...
dist_pred* dijkstra(adj_list* adj_list_in, int source) {
	...
	while (!empty_heap(heap_in)) {
		...
		while (list_in != NULL) {
			if (decrease_dist(heap_in, list_in->node,
					node_dist_out.dist + (list_in->dist)))
...
void print_dist_pred(adj_list* adj_list_in, int source) {
	...
	for (i = 0; i < num_nodes; i++) {
		printf("dist(%d,%d)=%d, pred(%d)=%d\n", source, i,
				dist_pred_out[i].dist, i, dist_pred_out[i].pred);
	}
}
...
int main(void) {
	...
	if (h->min != NULL) {
		...
		printf("heap vuoto");
	DECREASE_KEY(h, h->dict[9], 3);
	if (h->min != NULL) {
		SerializeFib(h->min);
		printf("\n");
	} else
		printf("heap vuoto");
	DECREASE_KEY(h, h->dict[8], 1);
	if (h->min != NULL) {
		SerializeFib(h->min);
		printf("\n");
	} else
